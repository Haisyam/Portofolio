...
fetch("../constants/Tools/Islami.json")
  .then((res) => res.json())
  .then((Islami) => {
    document.querySelector("#islami").innerHTML = Islami.map(
      (Islami) => `
      <article class="rounded-xl border-2 border-gray-100 bg-gray-400 mb-2">
          <div class="flex items-start gap-4 p-2">
            <a href="${Islami.link}" class="block shrink-0">
              <img
                alt="${Islami.imageAlt}"
                src="${Islami.image}"
                class="size-14 rounded-lg object-cover"
            ...
            <div>
              <h3 class="font-medium sm:text-lg">
                <a href="${Islami.link}" class="hover:underline"> ${Islami.namaTools} </a>
              </h3>
              ...
              <div class="py-2 flex md:hidden">
                <strong
                  class="-mb-[2px] -me-[2px] inline-flex items-center gap-1 rounded-ee-xl rounded-ss-xl ${Islami.warna} px-3 py-1.5 text-white"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="size-4"
                  ...
                  <span class="text-[10px] font-medium sm:text-xs">${Islami.status}</span>
                </strong>
              </div>
            </div>
          </div>
